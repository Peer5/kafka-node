...
Zookeeper.prototype.getOwnedPartitions = function (consumerId, groupId, topic, cb) {
    var that = this;
    ...
    this.client.getChildren(
        ...
        function (error, children) {
            if (error) {
                ...
                async.each(children, function(partition, callback) {
                    var partitionPath = path + '/' + partition;
                    that.client.getData(partitionPath, function(err, data) {
                        if (err) {
                            callback(err);
                        } else {
