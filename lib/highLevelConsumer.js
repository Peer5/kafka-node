    ...
    async.series([
            ...
            function (callback) {
                ...
                async.each(topics, function(topic, callback) {
                    debug("HighLevelConsumer %s fetching owned partitions for topic %s", self.id, topic);

                    // get all partitions we own
                    self.client.zk.getOwnedPartitions(self.id, self.options.groupId, topic, function(err, partitions) {
                        if (err) {
                            debug("HighLevelConsumer %s failed fetching partitions for topic %s", self.id, topic);
                            callback(err);
                        } else {
                            debug("HighLevelConsumer %s fetched %i owned partitions for topic %s", self.id, partitions.length, topic);
                            // for each partition we own, remove the ownership to avoid locking someone else
                            async.each(partitions, function(partition, delcb) {
                                debug("HighLevelConsumer %s deleting owned partition %i for topic %s", self.id, partition, topic);
                                self.client.zk.deletePartitionOwnership(self.options.groupId, topic, partition, function (err) {
                                    if (err) {
                                        debug("HighLevelConsumer %s failed deleting owned partition %i for topic %s", self.id, partition, topic);
                                        delcb(err);
                                    } else  {
                                        debug("HighLevelConsumer %s deleted owned partition %i for topic %s", self.id, partition, topic);
                                        delcb();
                                    }
                                });
                            }, function(err) {
            ...
            // Check that no new clients connected/disconnected
            function (callback) {
                debug("HighLevelConsumer %s checking clients map", self.id);
                self.client.zk.getConsumersPerTopic(self.options.groupId, function (err, obj) {
                    if (err) {
                        self.shouldRebalance = true;
                    }
                    if (!_.isEqual(consumerPerTopicMap, obj)) {
                        self.shouldRebalance = true;
                    }
                    callback();
                });
